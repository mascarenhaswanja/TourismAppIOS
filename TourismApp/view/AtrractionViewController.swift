...
class AtrractionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, AttractionCellDelegate {
    ...
    //  READ JSON from Static PATH: filepath = Bundle.main.path
    ...
    //  READ JSON from Dinamic PATH: paths = FileManager.default.urls
    ...
    func createPreferences() {
        ...
        do {
            ...
            let jsonData = try encoder.encode(userPreferences)  // use array appended elements
            
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                let paths = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)
                let finalPath = paths[0]
                let filename = finalPath.appendingPathComponent(fileName)
                try jsonString.write(to: filename, atomically:true, encoding: String.Encoding.utf8)  // WRITE
            }
            else {
    ...
    @IBAction func logoutPressed(_ sender: UIButton) {
        ...
        // need  image and text box programatically
//
